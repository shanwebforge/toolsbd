...
async function loadaiPosts() {
  ...
  const colRef = collection(db, "posts_ai");
  const snapshot = await getDocs(colRef);

  container.innerHTML = "";

  if (snapshot.empty) {
    container.innerHTML = "<p>No ai posts found.</p>";
    return;
  }
  ...
  snapshot.forEach(docSnap => {
    ...
    postCard.innerHTML = `
      ...
      <div class="post-content">
        <div class="post-user">
          ...
          <div><strong>${post.author}</strong><br/><span class="user-role">${post.role}</span></div>
        </div>
        <p class="post-description">${post.description.substring(0, 100)}...</p>
        <button class="read-more-btn" onclick="window.location.href='/blog/post.html?id=${postId}&cat=posts_ai'">Read More</button>
        <div class="post-actions-line">
          <div class="reactions">
            ...
            <span class="shareBtn" data-title="${post.title}" data-desc="${post.description}" data-url="/blog/post.html?id=${postId}&cat=posts_ai"><i class="fas fa-share-alt"></i> Share</span>
          </div>
    ...
    const docRef = doc(db, "posts_ai", postId);
  ...
  document.querySelectorAll('.likeBtn').forEach(btn => {
    btn.onclick = async () => {
      const ref = doc(db, "posts_ai", btn.dataset.id);
...
document.addEventListener("DOMContentLoaded", loadaiPosts);
