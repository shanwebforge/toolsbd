...
async function loadfreelanchingPosts() {
  ...
  if (snapshot.empty) {
    container.innerHTML = "<p>No freelancing posts found.</p>";
    return;
  }
  ...
  snapshot.forEach(docSnap => {
    ...
    postCard.innerHTML = `
      ...
      <div class="post-footer">
        ...
        <div class="post-stats">
          ...
          <span class="share" data-url="/blog/post.html?id=${postId}&cat=posts_freelanching"><i class="fas fa-share-alt"></i></span>
        </div>
      </div>
    `;
    ...
    postCard.addEventListener('click', e => {
      if (!e.target.closest('.like') && !e.target.closest('.dislike') && !e.target.closest('.share')) {
        window.location.href = `/blog/post.html?id=${postId}&cat=posts_freelanching`;
      }
    });
  });
  ...
  document.querySelectorAll('.like').forEach(btn=>{
    btn.onclick = async e=>{
      ...
      const ref = doc(db, "posts_freelanching", postId);
