...
async function loadIslamicPosts() {
  const container = document.getElementById("postsContainer");
  container.innerHTML = "<p>Loading posts...</p>";

  const colRef = collection(db, "posts_islamic");
  const snapshot = await getDocs(colRef);

  container.innerHTML = "";

  if (snapshot.empty) {
    container.innerHTML = "<p>No Islamic posts found.</p>";
    return;
  }
  ...
  snapshot.forEach(docSnap => {
    ...
    postCard.innerHTML = `
      ...
      <div class="post-content">
        ...
        <button class="read-more-btn" onclick="window.location.href='/blog/post.html?id=${postId}&cat=posts_islamic'">Read More</button>
        <div class="post-actions-line">
          <div class="reactions">
            ...
            <span class="shareBtn" data-title="${post.title}" data-desc="${post.description}" data-url="/blog/post.html?id=${postId}&cat=posts_islamic"><i class="fas fa-share-alt"></i> Share</span>
          </div>
    ...
    const docRef = doc(db, "posts_islamic", postId);
  ...
  document.querySelectorAll('.likeBtn').forEach(btn => {
    btn.onclick = async () => {
      const ref = doc(db, "posts_islamic", btn.dataset.id);
...
document.addEventListener("DOMContentLoaded", loadIslamicPosts);
