...
async function loadPosts(category) {
  ...
  if (snapshot.empty) {
    container.innerHTML = `<p>No ${category} posts found.</p>`;
    return;
  }
  ...
  snapshot.forEach(docSnap => {
    ...
    postCard.innerHTML = `
      ...
      <div class="post-footer">
        ...
        <div class="post-stats">
          ...
          <span class="share" data-url="/blog/post.html?id=${postId}&cat=${category}"><i class="fas fa-share-alt"></i></span>
        </div>
      </div>
    `;
    ...
    postCard.addEventListener('click', e => {
      if (!e.target.closest('.like') && !e.target.closest('.dislike') && !e.target.closest('.share')) {
        window.location.href = `/blog/post.html?id=${postId}&cat=${category}`;
      }
    });
  });
  ...
  document.querySelectorAll('.like').forEach(btn=>{
    btn.onclick = async e=>{
      ...
      const ref = doc(db, category, postId);
...
// Example usage:
document.addEventListener("DOMContentLoaded", ()=> loadPosts('posts_keyboard'));
