...
async function loadMediaPosts() {
  ...
  snapshot.forEach(docSnap => {
    ...
    postCard.innerHTML = `
      ...
      <div class="post-footer">
        ...
        <div class="post-stats">
          ...
          <span class="share" data-url="/blog/post.html?id=${postId}&cat=posts_media"><i class="fas fa-share-alt"></i></span>
        </div>
      </div>
    `;
    ...
    // Full card click except buttons
    postCard.addEventListener('click', e => {
      if (!e.target.closest('.like') && !e.target.closest('.dislike') && !e.target.closest('.share')) {
        window.location.href = `/blog/post.html?id=${postId}&cat=posts_media`;
      }
    });
  });
  ...
  document.querySelectorAll('.like').forEach(btn=>{
    btn.onclick = async e=>{
      ...
      const ref = doc(db, "posts_media", postId);
